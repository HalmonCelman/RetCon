...
#define SELECT()	PORTA &= ~(1<<PA0)		/* MMC CS = L */
#define	DESELECT()	PORTA |=  (1<<PA0)		/* MMC CS = H */
#define	MMC_SEL		!(PORTA &  (1<<PA0))	/* MMC CS status (true:selected) */

void INIT_SPI(void){
DDRB |= (1<<MOSI)|(1<<SCK)|(1<<PB4); //SS pin as an output - to working SPI
